...
function CertificateIdList({ type }) {
  ...
  return (
    <>
      {requests.length ? (
        viewDetails ? (
          <CertificateIdModal setViewDetails={setViewDetails} request={requestModal}/>
        ) : (
          <ul className="grid grid-cols-1 gap-5 min-[800px]:grid-cols-2 lg:grid-cols-3">
            {requests.map((request) => (
              <CertificateItem request={request} key={request._id} setViewDetails={setViewDetails} setRequestModal={setRequestModal}/>
            ))}
          </ul>
        )
      ) : (
